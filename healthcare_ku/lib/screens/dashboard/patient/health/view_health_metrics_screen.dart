...
class _ViewHealthMetricsScreenState extends State<ViewHealthMetricsScreen> {
  ...
  Future<void> _refreshMetrics() async {
    ...
    try {
      ...
      print('Fetching metrics for user: $userId');

      final newMetrics = await _firebaseService.getPatientHealthMetrics(userId);
      // Add print statement for debugging
      print('Fetched ${newMetrics.length} metrics');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Health Metrics History'),
        actions: [
          IconButton(
      ...
      body: _isLoading
          ...
          : metrics.isEmpty
              ? Center(child: Text('No health metrics recorded yet'))
              : RefreshIndicator(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => AddHealthMetricScreen(
                patientId: FirebaseAuth.instance.currentUser!.uid,
              ),
            ),
          );
          // Refresh the metrics after returning from the add screen
          _refreshMetrics();
        },
